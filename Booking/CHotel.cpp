...
void CHotel::print_descriere()
{
	cout << "Hotelul " << get_denumire() << " este situat in orasul " << get_locatie() + " si are "<<get_clasificare()<<" stele.";
...
void CHotel::list_informatii_camere()
{
	cout << "------------------------------------\nAti selectat Hotelul "<<get_denumire() + "." << endl;
	CUnitatiCazare::list_informatii_camere();
}

istream& operator>>(istream& in, CHotel& unitate)
{
...
ostream& operator<<(ostream& out, CHotel& unitate)
{
    out << "\n" << "Hotel" << "," << unitate.get_denumire() << "," << unitate.get_locatie() << "," << unitate.get_clasificare() << ",";
    int i = 1;
